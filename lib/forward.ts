...
export async function forwardToN8n(
  ...
  if (!url) {
    ...
    console.info("[forward] mock-mode (LE_OPTIN_WEBHOOK_URL unset)", { scope, payload });
  ...
  try {
    ...
    if (!response.ok) {
      // Surface n8n's verdict in Vercel function logs — we map this to
      // generic 500 for the public response, but ops needs the real status
      // to triage 4xx (config/whitelist issues) vs 5xx (n8n outage).
      console.warn("[forward] n8n non-2xx", {
        scope,
    ...
    console.warn("[forward] n8n fetch failed", { scope, reason });
    return {
      ok: false,
      status: 0,
      reason,
